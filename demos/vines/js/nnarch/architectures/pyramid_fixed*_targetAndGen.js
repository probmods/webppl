...
// This version is not quite totally fixed, because it has learnable out-of-bounds values
		...
		for (var i = 0; i < nPyramidLevels; i++) {
			...
			for (var wy = cy - 1; wy <= cy + 1; wy++) {
				for (var wx = cx - 1; wx <= cx + 1; wx++) {
					...
					features[fidx] = inbounds ? targetImg.data[imgidx] : outOfBoundsTarget;
					fidx++;
					

					//Adding image so far to features
					features[fidx] = inbounds ? imgSoFar.data[imgidx] : outOfBoundsSoFar;
					fidx++;				
				}
			}
		}
