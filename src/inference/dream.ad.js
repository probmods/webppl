...
module.exports = function(env) {
  ...
  function SDREAM(wpplFn, s, a, options, state, step, cont) { //params, 
    ...
    // points to the current record, containing a completed trace
  ...
  SDREAM.prototype = {
      ...
      return util.cpsLoop(
          ...
          function(i, next) {
            var samplingPass = function(i, cont) {
              ...
              this.currRecord = {trace: trace, targetScore: 0, observations: []};
    ...
    estimateGradient: function(cont) {
      ...
      return this.wpplFn(_.clone(this.s), function() {
        ...
        var logp = ad.value(this.currRecord.targetScore); //trace.score would include the logp also for outside mapData
    ...
    sample: function(s, k, a, dist, options) {
      ...
      if (this.isSamplingPass) {
        ...
        if (this.isInsideMapData()) {
          this.currRecord.targetScore += distribution.score(val);
        }
      }
      else {
    ...
    observe: function(s, k, a, dist, val) {
      if (this.isSamplingPass) {
        ...
        if (this.isInsideMapData()) {
          this.currRecord.targetScore += dist.score(hallucinatedVal);
        }
